...
import { configPromptArgs, filterPromptArgs, resolvePrompt } from "../lib/resolve-prompt.js";
...
export async function runAddressReview(opts: AddressReviewOpts): Promise<AddressReviewResult> {
    if (opts.round > opts.maxRounds) {
        throw new Error(
            `Invalid address-review round range: round (${opts.round}) must be less than or equal to maxRounds (${opts.maxRounds})`,
        );
    }
    ...
    return {
        ...
        roundCapped: latestRoundCapped,
        roundsRun,
    };
}

async function runAddressReviewRound(
    opts: AddressReviewOpts,
): Promise<Omit<AddressReviewResult, "roundsRun">> {
    ...
    for (const { scored } of withThreads) {
        ...
        console.log(
            `[address-review]   score=${scored.score} tier=${scored.tier} signals=[${signalNames}]`,
        );
    }

    // 3. Partition: Auto/Confirm vs HITL
    const autoConfirm = withThreads.filter(
        (c) => c.scored.tier === "auto" || c.scored.tier === "confirm",
    );
    ...
    if (autoConfirm.length > 0) {
        console.log(
            `[address-review] Fixing ${autoConfirm.length} auto/confirm comment(s) via Sandcastle...`,
        );
        ...
        try {
            const promptFile = await resolvePrompt({
                name: "address-review",
                config,
                repoDir,
                templatesDir,
            ...
            const touchedComments = autoConfirm.filter(
                (c) => c.scored.comment.path && touchedFiles.has(c.scored.comment.path),
            );
            if (touchedComments.length > 0) {
                resolveThreads({ threadIds: touchedComments.map((c) => c.threadId), cwd: repoDir });
            }
            const unresolvedAfterResolution = new Set(
                fetchPrComments({ prNumber, cwd: repoDir }).comments.review_threads.map(
                    (thread) => thread.commentId,
                ),
            );
            for (const c of autoConfirm) {
                if (
                    c.scored.comment.path &&
                    touchedFiles.has(c.scored.comment.path) &&
                    !unresolvedAfterResolution.has(c.commentId)
                ) {
                    fixedCount++;
                    results.push({
                        body: c.scored.comment.body,
    ...
    for (const c of hitl) {
        try {
            ...
            console.log(
                `[address-review] Deferred to #${result.issueNumber}: ${c.scored.comment.body.slice(0, 50)}`,
            );
        } catch (err: unknown) {
            const message = err instanceof Error ? err.message : String(err);
...
function getBranch(opts: { prNumber: string; cwd: string }): string {
    return shFile(
        "gh",
        ["pr", "view", opts.prNumber, "--json", "headRefName", "--jq", ".headRefName"],
        opts.cwd,
    ).trim();
}
...
function changedFilesForCommit(opts: { sha: string; cwd: string }): string[] {
    try {
        const output = shFile(
            "git",
            ["diff-tree", "--no-commit-id", "--name-only", "-r", opts.sha],
            opts.cwd,
        );
        ...
        const message = err instanceof Error ? err.message : String(err);
        console.warn(
            `[address-review] Could not inspect changed files for ${opts.sha}: ${message}`,
        );
        return [];
    }
}
