...
export async function runImplementPr(opts: {
    prNumber: string;
    repoDir: string;
    model?: string;
    templatesDir?: string;
    ...
    console.log(
        `[implement-pr] Linked issue: ${issueNumber === "(none)" ? "none" : `#${issueNumber} — ${issueTitle}`}`,
    );
    ...
    const extractionPrompt = readFileSync(path.join(extractionsDir, "implement-pr.md"), "utf8");
