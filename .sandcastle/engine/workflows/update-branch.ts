...
import {
    resolveDefaultExtractionsDir,
    resolveDefaultTemplatesDir,
} from "../lib/default-template-paths.js";
...
export async function runUpdateBranch(opts: {
    ...
    console.log(
        `[update-branch] Merge produced conflicts in ${mergeResult.conflicts.length} file(s) — invoking agent.`,
    );
    ...
    const extractionPrompt = readFileSync(path.join(extractionsDir, "update-branch.md"), "utf8");
