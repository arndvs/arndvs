...
export function postReview(opts: PostReviewOpts): PostReviewResult {
    ...
    const validInlineComments = inlineComments.filter((c) => {
        ...
        if (!fileLines) {
            console.warn(
                `${prefix} Dropping inline comment for ${c.path}:${c.line} — file not in diff`,
            );
            return false;
        }
        if (!fileLines[c.side].has(c.line)) {
            console.warn(
                `${prefix} Dropping inline comment for ${c.path}:${c.line} ${c.side} — line not in diff hunks`,
            );
            return false;
        }
        return true;
    ...
    const threadIdByCommentId = new Map(
        prComments.review_threads.map((c) => [c.commentId, c.threadId]),
    );
    const validReplies = threadReplies.filter((r) => {
        if (!validReplyIds.has(r.commentId)) {
            console.warn(
                `${prefix} Dropping reply for commentId=${r.commentId} — not in fetched threads`,
            );
            return false;
        }
        return true;
    ...
    const effectiveReviewBody =
        opts.reviewBody ??
        (validInlineComments.length > 0 ? "Addressed review feedback." : undefined);
