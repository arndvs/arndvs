 ...
 */
export function deferToIssue(opts: {
    ...
    if (existing) {
        postThreadReply({
            threadId,
            body: `Deferred to #${existing.number} — score ${score}/100 (HITL tier)`,
            cwd,
        });
    ...
    postThreadReply({
        threadId,
        body: `Deferred to #${issueNumber} — score ${score}/100 (HITL tier)`,
        cwd,
    });
    resolveThread({ threadId, cwd });
...
function findExistingIssue(opts: {
    title: string;
    owner: string;
    repo: string;
    cwd: string;
