...
export const TRANSITIONS: ReadonlyMap<string, ReadonlySet<string>> = new Map([
    ...
    ["agent:implement", new Set(["agent:pr-open", "agent:implement-prd"])],
    ...
    ["agent:implement-prd", new Set(["agent:implement-prd", "agent:review", "agent:implement"])],
    // PR verdict paths
...
export function validateTransition(
    ...
    for (const [a, b] of MUTUAL_EXCLUSIONS) {
        if (resultSet.has(a) && resultSet.has(b)) {
            errors.push(`Mutual exclusion violated: "${a}" and "${b}" cannot coexist.`);
        }
    }
