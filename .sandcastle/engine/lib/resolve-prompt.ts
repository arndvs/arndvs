...
export async function resolvePrompt(opts: ResolvePromptOpts): Promise<string> {
    ...
    const overrideDir = isAbsolute(config.promptDir)
        ? config.promptDir
        : join(repoDir, config.promptDir);
    ...
    throw new Error(
        `Prompt not found: ${filename} — checked override (${overridePath}) and template (${templatePath})`,
    );
}
...
export function filterPromptArgs(
    promptFile: string,
