...
export type WorkflowRunner = (opts: {
    args: CliArgs;
    repoDir: string;
    templatesDir: string;
}) => Promise<void>;
...
const workflows: Record<string, WorkflowRunner> = {
    ...
    "update-branch": async ({ args, repoDir, templatesDir }) => {
        ...
        const result = await runUpdateBranch({
            prNumber: args.pr,
            branch: args.branch,
    ...
    "implement-prd": async ({ args, repoDir, templatesDir }) => {
        ...
        if (!args.subIssueNumber)
            throw new Error("implement-prd requires --sub-issue-number <number>");
    ...
    "architecture-review": async ({ repoDir, templatesDir }) => {
        ...
        const outputDir = outputDirPath();
        fs.writeFileSync(
            join(outputDir, "architecture_review_output.json"),
