...
const TEST_PATH_PATTERNS = [/\.test\.\w+$/, /\.spec\.\w+$/, /__tests__\//, /\bfixtures?\b/];
...
function collectSignals(comment: PrComment): Signal[] {
    ...
    if (
        comment.filesAffected != null &&
        comment.linesAffected != null &&
        comment.filesAffected <= 1 &&
        comment.linesAffected <= 10
    ) {
