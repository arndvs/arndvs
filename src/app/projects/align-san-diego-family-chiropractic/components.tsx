    ...
    >
      ...
      >
        {data.subsystems.map((sub) => {
          const Icon = subsystemIcons[sub.title] ?? Activity
          return (
...
export function DeepDiveSection({ data }: { data: DeepDive }) {
  return (
    ...
    >
      ...
      <motion.div
        variants={itemVariants}
        className="rounded-lg border-l-4 border-l-cyan-500 bg-muted/50 p-6"
      >
          ...
          <motion.div key={m.label} variants={cardVariants}>
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-2xl font-bold text-cyan-500/80">
                  <AnimatedCounter
                    value={m.value}
                    prefix={m.prefix}
                    suffix={m.suffix}
